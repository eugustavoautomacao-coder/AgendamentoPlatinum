...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: profile, error: profileError } = await supabaseClient.from('users').select('tipo').eq('id', user.id).single();
    ...
    if (tipo === 'cliente' && !salao_id) {
      return new Response(JSON.stringify({
        error: 'Salão é obrigatório para clientes'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    if (tipo === 'system_admin') {
      return new Response(JSON.stringify({
        error: 'Cadastro de superadmin não permitido'
      }), {
        status: 403,
        headers: {
          ...corsHeaders,
    ...
    console.log('Auth createUser result:', {
      authData,
      authError
    });
    if (authError) {
    ...
    if (tipo === 'funcionario') {
      ...
      console.log('Employee insert result:', {
        error: employeeError
      });
      if (employeeError) {
        // rollback
