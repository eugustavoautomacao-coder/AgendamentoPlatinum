...
serve(async (req) => {
  ...
  try {
    ...
    // 1. Primeiro, exclui da tabela public.users
    ...
    if (deleteUserError) {
      ...
      return new Response(
        JSON.stringify({ error: `Failed to delete user from users table: ${deleteUserError.message}` }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    // 2. Depois, exclui da tabela auth.users
    ...
    return new Response(
      JSON.stringify({ success: true, message: 'User deleted successfully from both tables' }),
